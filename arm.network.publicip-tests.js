    ...
    describe('publicip', function () {
      it('create', function (done) {
		createGroup(function(){
			suite.execute('network publicip create %s %s  --domain-name %s  --location %s', groupName, publicipName, dnsPrefix, location,  function (result) {
	  ...
	  it('show', function (done) {
		  suite.execute('network publicip show %s %s --json', groupName, publicipName, function (result) {
			result.exitStatus.should.equal(0);
		...
		it('delete', function (done) {
		  suite.execute('network publicip delete %s %s --quiet', groupName, publicipName, function (result) {
          result.exitStatus.should.equal(0);
		  done();
		});
	  });
