  ...
  describe('network', function () {
    ...
    describe('vnet', function () {
		...
		it('export', function (done) {
		  suite.execute('network vnet export %s %s %s --json', groupName, vnetPrefix,networkconfig, function (result) {
            result.exitStatus.should.equal(0);
			done();
			});
		});
		...
		it('import', function (done) {
		  suite.execute('network vnet import %s %s %s', groupName, vnetPrefix, networkconfig, function (result) {
			  console.log(result);
			result.exitStatus.should.equal(0);
			done();
			});
